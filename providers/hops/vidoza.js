function extract(ctx) {
  ...
  return ctx.fetch(url, { headers: headers }).then(function (r) { return r.text(); }).then(function (html) {
    if (cf(html) || /File Not Found|File was deleted/i.test(html)) return [];
    var src = (html.match(/sourcesCode:\s*\[\s*\{\s*src:\s*["'](https?:[^"']+)/) || html.match(/src:\s*["'](https?:[^"']+\.(?:mp4|m3u8)[^"']*)["']/) || [])[1];
    return src ? [row(src, { name: 'Vidoza' })] : [];
