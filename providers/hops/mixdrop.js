function extract(ctx) {
  ...
  return ctx.fetch(url, { headers: headers }).then(function (r) { return r.text(); }).then(function (html) {
    if (cf(html) || /can't find the (file|video)/i.test(html)) return [];
    var unpacked = (globalThis.__engineUnpack && globalThis.__engineUnpack(html)) || html;
    var wurl = (unpacked.match(/MDCore\.wurl\s*=\s*["']([^"']+)["']/) || html.match(/MDCore\.wurl\s*=\s*["']([^"']+)["']/) || [])[1];
    if (!wurl) return [];
    if (wurl.indexOf('//') === 0) wurl = 'https:' + wurl;
    return [row(wurl, { name: 'MixDrop', referer: url })];
