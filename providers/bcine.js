...
function extract(ctx) {
  ...
  var tokenApi = String(cfg.tokenApi || SPECS.tokenApi).replace(/\/$/, '');
  ...
  var directHls = generateDirectHlsUrl(tmdbId, season, episode) + '?' + vParam;
  var rows = [
    {
      url: directHls,
      name: 'Bcine · Direct Master',
      quality: '1080p',
      headers: {
        'User-Agent': ua,
    ...
    .then(function (data) {
      ...
      return Promise.all(
        servers.map(function (srv) {
          return ctx
            .fetch(tokenApi + srv.endpoint + '?' + query, {
              headers: {
                'User-Agent': ua,
                Referer: tokenApi + '/',
            ...
            .then(function (j) {
              var sUrl = j && j.url ? String(j.url) : '';
              if (!sUrl || sUrl.indexOf('.m3u8') < 0) return null;
              return {
                url: sUrl,
                name: 'Bcine · ' + srv.name,
                quality: '1080p',
                headers: { 'User-Agent': ua, Referer: tokenApi + '/' },
              };
            })
            .catch(function () {
              return null;
